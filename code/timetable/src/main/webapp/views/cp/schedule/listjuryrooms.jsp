	...
	<form:form id="form-add" action="${baseUrl}/cp/save-a-jury-members.html" method="POST" role="form" accept-charset="UTF-8">
		...
		<div class="panel panel-default">
				<div class="panel-heading"><h4></h4> 
				    <button type="button" class="form btn btn-primary" onclick="addJuryMember();" id="choose">Thêm</button>
				</div>
...
<script type="text/javascript">
	...
	function addJuryMember()
	{
		...
		var savingJuryMemberUrl = "${baseUrl}/cpservice/savejuryaroom.html";
		...
		if (sRoomCode.length > 0 && sDefenseSessionCode.length > 0 ) {
			 $.ajax({
					...
					success: function(html) {    
						$("#status").html( html );
						window.location = baseUrl + "/cp/juryrooms.html";
					}
				});
		}else{
			/*
	...
	function v_fRemoveJuryRoom(the_sz_JuryRoomCode){
		var r = confirm("Bạn có muốn xóa bản ghi này ?");
		if (r == true) {
			var sRemoveAJuryRoomUrl = baseUrl + "/cp/remove-a-jury-room/"+the_sz_JuryRoomCode+".html";
			window.location = sRemoveAJuryRoomUrl;
		} else {
		    return false;
		}
	}
